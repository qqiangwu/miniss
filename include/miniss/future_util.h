...
template <class Fut>
struct When_all_state : public std::enable_shared_from_this<When_all_state<Fut>> {
    ...
    {
        for (auto& f: futures_) {
            f = f.then_wrapped([s = this->shared_from_this()](auto&& fut){
                return std::move(fut);
            });
        }
...
template <class Fut>
future<std::vector<Fut>> when_all(std::vector<Fut>&& futs)
{
